...
function App() {
  ...
  const [upperClass, setUpperClass] = useState('check');
  const [leftClass, setLeftClass] = useState("first-left-class")
  ...
  const search = async (e) => {
    ...
    setSearchInput("");
    setUpperClass("container stats-container");
    setLeftClass("left-panel");
  }
  ...
  const validSongFilter = (value, type) => {
    if (value < minTrackCount() && value > maxTrackCount()){
      ...
      alert("Invalid! You cannot set lower bound greater than upper bound.")
    } else if (type == 'u' && value < lowerBound){
      alert("Invalid! You cannot set upper bound lesser than lower bound.")
    } else{
      return true;
    }
    return false;
    ...
    <div className="App">
      ...
      <div className='right-panel'>
        <div className={upperClass}>
          <div className="stat-card">{artistData ? "Average Rating: " + artistData.popularity : ""}</div>
          <div className="stat-card">{albums.length !== 0 ? "Albums Size: " + albums.length : ""}</div>
